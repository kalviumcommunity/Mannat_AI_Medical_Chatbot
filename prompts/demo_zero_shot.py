...
def naive_local_answer(context: str, question: str) -> str:
    """A deterministic fallback that attempts to answer using context."""
    ctx = context.lower()
    q = question.lower()
...
def call_groq_model(model_name: str, api_key: str, prompt: str) -> str:
    """Attempt to call Groq via langchain_groq.ChatGroq."""
    try:
        from langchain_groq import ChatGroq
    except Exception:
        raise RuntimeError("Missing dependency: langchain_groq is not installed. Install with pip install langchain-groq.")
